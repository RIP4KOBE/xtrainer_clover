...
from ModelTrain.dp.vis_utils import filter_points_by_bounds, batch_transform_points


def add_to_visualize_buffer(visualize_buffer, visualize_points, visualize_colors):
    assert visualize_points.shape[0] == visualize_colors.shape[
        0], f'got {visualize_points.shape[0]} for points and {visualize_colors.shape[0]} for colors'
...
class Visualizer:
    ...
    def show_pointcloud(self, points, colors):
        ...
        pcd.points = o3d.utility.Vector3dVector(
            points.astype(np.float64))  # float64 is a lot faster than float32 when added to o3d later
    ...
    def visualize_subgoal(self, subgoal_pose):
        ...
        transformed_collision_points = batch_transform_points(collision_points_centered,
                                                              subgoal_pose_homo[None]).reshape(-1, 3)
    ...
    def visualize_path(self, path):
        ...
        transformed_collision_points = batch_transform_points(collision_points_centered, poses_homo).reshape(-1,
                                                                                                             3)  # (num_poses, num_points, 3)
        ...
        for t in range(path_length):
            ...
            collision_points_colors[t] = (1 - whitening_coef) * collision_points_colors[t] + whitening_coef * np.array(
                [1, 1, 1])
