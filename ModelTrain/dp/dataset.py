...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.state_noise = state_noise
        self.memmap_loader_path = data.get("memmap_loader_path", None)
        self.memmap_loader = None  #不直接初始化 MemmapLoader
        self.representation_type = representation_type
        self.transform = transform
        self.get_img = get_img
        self.load_img = load_img
    ...
    def _initialize_memmap_loader(self):
        if self.memmap_loader is None and self.memmap_loader_path:
            self.memmap_loader = MemmapLoader(self.memmap_loader_path)  # ✅ 动态加载

    def __len__(self):
        return len(self.indices)

    def read_img(self, image_pathes, idx):
        self._initialize_memmap_loader()
        if self.memmap_loader is not None:
            # using memmap loader
            indices = range(idx, idx + self.obs_horizon)
            data = self.memmap_loader[indices]
